...
export default async function ProductsPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <p className="mt-1 text-muted">كل منتجاتنا مطبوعة عند الطلب بجودة عالية</p>
      ...
      <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <form action="/products" className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-line bg-surface py-2.5 pr-11 pl-4 text-sm outline-none transition-colors focus:border-primary md:w-64"
          />
      ...
      {products.length === 0 ? (
        <div className="mt-16 rounded-2xl border border-dashed border-line bg-surface p-12 text-center">
          ...
          <Link
            href="/products"
            className="mt-5 inline-block rounded-xl bg-primary px-6 py-3 font-bold text-white transition-colors hover:bg-primary-hover"
          >
            عرض كل المنتجات
          </Link>
        </div>
